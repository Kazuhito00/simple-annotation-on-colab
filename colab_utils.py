...
def draw_bbox(image_urls, callbackId):  # pylint: disable=invalid-name
    ...
    for image in image_urls:
        if isinstance(image, np.ndarray):
            ...
            raise TypeError('Image has unsupported type {}.'.format(
                type(image)))
...
def draw_polygon(image_urls, callbackId):  # pylint: disable=invalid-name
    """Open the Polygon UI and send the results to a callback function.
    ...
    js = Javascript('''
        async function load_image(imgs, callbackId) {
            ...
            // on add, deletes the last bounding box
            addButton.textContent = "add polygon";
            ...
            // on delete, deletes the last bounding box
            deleteButton.textContent = "undo polygon";
            deleteButton.onclick = function(){
                if (points.length > 0) {
                    points = [];
                } else {
            ...
            submit.onclick = function(){
                ...
                // send box data to callback fucntion
                google.colab.kernel.invokeFunction(callbackId, [allPolygons], {});
            }

            // init template for point
            const point = {
                x: 0,
                y: 0,
            };
            ...
            // the array of all rectangles
            let pointsList = allPolygons[curr_image];
            ...
            function handleMouseDown(e) {
                // on mouse click set point the cursor
                currentPoint = oMousePos(canvas_img, e);

                const mousePoint = Object.create(point);
                mousePoint.x = currentPoint.x / image.width;
                mousePoint.y = currentPoint.y / image.height;
                if (!e.shiftKey) {
            ...
            function draw() {
                ...
                for (let index = 0 ; index < pointsList.length ; index++) {
                    ...
                    for (let i = 0 ; i < pointsList[index].length ; i++) {
                        console.log(pointsList[index].length);
                        console.log(pointsList[index][i]);
                        ctx.beginPath();
                        ctx.arc(pointsList[index][i].x * image.width, pointsList[index][i].y * image.height, 1, 0*Math.PI, 2*Math.PI);
                        ctx.stroke();
                    }
                    for (let i = 0 ; i < pointsList[index].length ; i++){
                        if (i == 0) {
                            ctx.beginPath(pointsList[index][i].x * image.width, pointsList[index][i].y * image.height);
                        }
                        ctx.lineTo(pointsList[index][i].x * image.width, pointsList[index][i].y * image.height);
                    }
                    ctx.closePath();
                    ctx.stroke();
                }
                ...
                for (let i = 0 ; i < points.length ; i++){
                    ctx.beginPath();
                    ctx.arc(points[i].x * image.width, points[i].y * image.height, 1, 0*Math.PI, 2*Math.PI);
                    ctx.stroke();
                }

                for (let i = 0 ; i < points.length ; i++){
                    if (i == 0) {
                        ctx.beginPath(points[i].x * image.width, points[i].y * image.height);
                    }
                    ctx.lineTo(points[i].x * image.width, points[i].y * image.height);
                }
                ctx.closePath();
                ctx.stroke();
            }
            ...
            function resetcanvas(){
                ...
                image.onload = function() {
                    ...
                    // draw bounding boxes
                    pointsList = allPolygons[curr_image];
                    draw();
                };
            }
...
def annotate_polygon(
        imgs: List[Union[str, np.ndarray]],  # pylint: disable=invalid-name
        polygon_storage_pointer: List[np.ndarray],
        callbackId: str = None):
    """Open the polygon UI and prompt the user for input.
    ...
    Args:
        ...
        box_storage_pointer: list[np.ndarray]
        Destination list for polygon arrays. Each array in this list
        corresponds to one of the images given in imgs. The array is a
        N x 4 array where N is the number of polygons given by the user
        for that particular image. If there are no polygons for an image,
        None is used instead of an empty array.
    ...
    def dictToList(input_polygon):  # pylint: disable=invalid-name
        """Convert bbox.

        This function converts the dictionary from the frontend into a list

        Args:
            input_polygon:

        Returns:
            A list with polygons coordinates.
        """
        point_list = []
    ...
    def callbackFunction(annotations: List[List[Dict[str, float]]]):  # pylint: disable=invalid-name
        ...
        Args:
            ...
            [
                ...
                [
                    // stuff for polygon 1
                    [
                        // stuff for polygon point 1
                        [x, y],
                        // stuff for polygon point 2
                        [x, y],
                        ...
                    ],
                    // stuff for polygon 2
        ...
        # reset the boxes list
        nonlocal polygon_storage_pointer
        polygons: List[np.ndarray] = polygon_storage_pointer
        polygons.clear()
        ...
        for annotations_per_img in annotations:
            ...
            if rectangles_as_arrays:
                polygons.append(np.stack(rectangles_as_arrays))
            else:
                polygons.append(None)

        # output the annotations to the errorlog
        with output.redirect_to_element('#errorlog'):
            display('--polygons array populated--')

    output.register_callback(callbackId, callbackFunction)
    draw_polygon(imgs, callbackId)
